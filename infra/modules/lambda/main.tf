...
resource "aws_lambda_function" "orders" {
  function_name = "${local.name_prefix}-orders-handler${local.s}"
  role          = var.lambda_role_arn
...
resource "aws_lambda_function" "users" {
  function_name = "${local.name_prefix}-users-handler${local.s}"
  role          = var.lambda_role_arn
...
# CARTS LAMBDA
############################################
resource "aws_lambda_function" "carts" {
  function_name = "${local.name_prefix}-carts-handler${local.s}"
  role          = var.lambda_role_arn
  ...
  filename         = var.carts_zip_path
  source_code_hash = filebase64sha256(var.carts_zip_path)
